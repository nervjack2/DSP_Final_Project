...
clean_voice_dir = '../Train/clean_voice'
noise_voice_dir = '../Train/noise_voice'
sample_rate = 8000
frame_length = 8064
stride_noise = 5000
stride_clean = 8064
...
if TRAIN:
    ...
    librosa.output.write_wav('../Train/sound/' + 'noisy_voice.wav', noisy_voice[0, :], sample_rate)
    voice = blend_voice.reshape(1, num_sample * frame_length)
    librosa.output.write_wav('../Train/sound/' + 'voice.wav', voice[0, :], sample_rate)
    noise = blend_noise.reshape(1, num_sample * frame_length)
    librosa.output.write_wav('../Train/sound/' + 'noise.wav', noise[0, :], sample_rate)
    ...
    np.save('../Train/spectrogram/' + 'voice_amp_db', amp_voice)
    np.save('../Train/spectrogram/' + 'noise_amp_db', amp_noise)
    np.save('../Train/spectrogram/' + 'noisy_voice_amp_db', amp_noisy_voice)

    np.save('../Train/spectrogram/' + 'voice_pha_db', pha_voice)
    np.save('../Train/spectrogram/' + 'noise_pha_db', pha_noise)
    np.save('../Train/spectrogram/' + 'noisy_voice_pha_db', pha_noisy_voice)
else:
    noisy_voice = blend_noisy_voice.reshape(1, num_sample * frame_length)
    librosa.output.write_wav('../demo/' + 'noisy_voice.wav', noisy_voice[0, :], sample_rate)
    voice = blend_voice.reshape(1, num_sample * frame_length)
    librosa.output.write_wav('../demo_ans/' + 'voice.wav', voice[0, :], sample_rate)
    noise = blend_noise.reshape(1, num_sample * frame_length)
    librosa.output.write_wav('../demo_ans/' + 'noise.wav', noise[0, :], sample_rate)